...
int
hex2raw( const char *hex, const char *raw )
{
    uint32_t start, end;
    uint8_t *buffer;
    FILE *out;
...
int
bin2hex( const char *bin, const char *hex )
{
    FILE *in;
    uint8_t *buffer;
    ...
    ifi_bin_read(bin, NULL, 0x1FFFF, &start, &end);
    printf("s: %.06X e: %.06X\n", start, end);
    
    buffer = (uint8_t *)malloc( end );
    memset(buffer, 0xFF, end);
    ifi_bin_read(bin, buffer, end, &start, &end);
...
int main( int argc, char **argv )
{
    ...
    while( (ch = getopt_long(argc, argv, "hbrv", longopts, NULL)) != -1 ) {
        ...
        case 'v':
            ...
            } else printf("HEX file specified is valid!\n");
            
            return 0;
        
        case 'w':
            printf("Rigel INHEX32 conversion utility, version %d.%d.\n",
